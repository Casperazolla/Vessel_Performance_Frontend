...
// tpd for one vessel at an ARBITRARY (draught, speed) operating point.

// Same nearest-neighbour logic as tpdAtDesign, but the point comes from the

// user's dropdown selection instead of the vessel's design reference.

function tpdAtPoint(fuelData, draught, speed) {
  ...
  if (Number.isFinite(d)) {
    ...
    rec = recs.reduce((a, b) =>
      ...
      Math.abs(Number(b.draught) - d) < Math.abs(Number(a.draught) - d) ? b : a
  ...
  const targetSpeed = Number.isFinite(s) ? s : Math.max(...rec.speed.map(Number));

  const val = rec.fuel_t_per_day[nearestIndex(rec.speed, targetSpeed)];

  return Number.isFinite(val) ? val : null;

}
 
// Fleet minimum design draught/speed across the analysed vessels.

function fleetMinRef(list) {
  ...
  list.forEach((v) => {
    ...
    const { draught, speed } = vesselRef(v);

    if (Number.isFinite(draught)) minD = Math.min(minD, draught);

    if (Number.isFinite(speed)) minS = Math.min(minS, speed);

  });

  return {

    draught: Number.isFinite(minD) ? minD : null,

    speed: Number.isFinite(minS) ? minS : null,

  };

}
 
// [min, min-1, ... , min-5], dropping any non-positive value.

function buildDescendingOptions(minVal, count = 5) {
  ...
  for (let i = 0; i <= count ; i++) {
    ...
    const val = Math.round((minVal - i) * 100) / 100;
...
// [min-5 ... min ... min+5], dropping any non-positive value.
function buildSpeedOptions(centerVal, down = 5, up = 5) {

  if (!Number.isFinite(centerVal)) return [];

  const out = [];

  for (let i = down; i >= 1; i--) {

    const val = Math.round((centerVal - i) * 100) / 100;

    if (val > 0) out.push(val);

  }

  out.push(Math.round(centerVal * 100) / 100);
...
const fmtNum = (n) => (Number.isInteger(n) ? String(n) : String(Number(Number(n).toFixed(2))));
...
function FleetBenchmark({ ok = [], fuelByImo = {} }) {
  ...
  // Operating-point override. "" = use each vessel's own design point (default,

  // identical to the previous behaviour). A numeric value overrides that axis.

  const [selDraught, setSelDraught] = useState("");

  const [selSpeed, setSelSpeed] = useState("");
 
  // Fleet minimum design draught/speed -> tops of the dropdowns.

  const fleetMin = useMemo(() => fleetMinRef(ok), [ok]);

  const draughtOptions = useMemo(() => buildDescendingOptions(fleetMin.draught), [fleetMin.draught]);

  const speedOptions = useMemo(() => buildSpeedOptions(fleetMin.speed), [fleetMin.speed]);
 
  const overrideActive = selDraught !== "" || selSpeed !== "";
  ...
  const { vessels, skipped } = useMemo(() => {
    ...
    ok.forEach((v) => {
      ...
      const ref = vesselRef(v);

      // Selected value wins per-axis; an axis left on "Design" keeps the

      // vessel's own design value, so a partial selection still makes sense.

      const useDraught = selDraught !== "" ? Number(selDraught) : ref.draught;

      const useSpeed = selSpeed !== "" ? Number(selSpeed) : ref.speed;

      const tpd = overrideActive

        ? tpdAtPoint(fuelByImo[v.imo], useDraught, useSpeed)

        : tpdAtDesign(fuelByImo[v.imo], ref);
  ...
  }, [ok, fuelByImo, selDraught, selSpeed, overrideActive]);
...
<span style={{ width: 10, height: 10, background: C.critical, display: "inline-block", borderRadius: "50%" }} /> Above benchmark
          ...
          {/* Operating-point selectors: fleet minimum design value down to (min - 5) */}
<div style={{
            ...
            background: "#f8fafc", border: `1px solid ${C.borderCard}`, borderRadius: 10,

          }}>
<span style={{ fontSize: 11, fontWeight: 700, color: C.textSecondary, letterSpacing: "0.04em" }}>
...
<select
                ...
                onChange={(e) => setSelDraught(e.target.value)}

                style={{ padding: "6px 8px", borderRadius: 8, background: "white", border: `1px solid ${C.border}`, color: "black", fontSize: 12 }}
>
<option value="">Design</option>

                {draughtOptions.map((d) => (
<option key={d} value={d}>{fmtNum(d)} m</option>
...
<select
                ...
                onChange={(e) => setSelSpeed(e.target.value)}
...
<option value="">Design</option>
                ...
                {speedOptions.map((s) => (
<option key={s} value={s}>{fmtNum(s)} kn</option>
...
<button
                ...
                onClick={() => { setSelDraught(""); setSelSpeed(""); }}

                style={{ padding: "6px 10px", borderRadius: 8, border: `1px solid ${C.border}`, background: "white", color: C.accent, fontSize: 11, fontWeight: 700, cursor: "pointer" }}
>
            ...
            <span style={{ marginLeft: "auto", fontSize: 11, color: C.textMuted }}>
              ...
              {overrideActive
                ...
                ? `Showing ${selDraught !== "" ? `${fmtNum(Number(selDraught))} m` : "design draught"} · ${selSpeed !== "" ? `${fmtNum(Number(selSpeed))} kn` : "design speed"}`

                : "Showing each vessel at its design point"}

              {fleetMin.draught != null && (
<> · fleet min design {fmtNum(fleetMin.draught)} m{fleetMin.speed != null ? ` / ${fmtNum(fleetMin.speed)} kn` : ""}</>
